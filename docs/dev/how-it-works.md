...
`AllTransactionsView` `@Query`s `Transaction` and `Income`, filters with `TransactionAnalytics` + `PeriodFilterMenu.swift`. Rows: `TransactionRows.swift`. Tap → `TransactionDetailView.swift` (category / multiplier / rail / learn rule). Save sets locks so Sync won’t overwrite. **Needs review:** `ReviewQueueView.swift`. **Import:** JSON in `ContentView.swift`; Apple Card CSV in `Features/Import/AppleCardCSVImport.swift` (payment method Apple Card, `AppleCardAccount.swift`; monthly installments → `Installment`, credits → `Refund`, skip $0 / Daily Cash Adjustment). Category charts: `CategorySpendView.swift` + `Shared/UI/CategorySpendChart.swift`. Plaid `classify`: on a **credit** account, money-in that is not a bill pay is `.adjustment` (Refund), not Income. `LOAN_DISBURSEMENTS` / “my loan” titles are Loan (not earnings). Weak categories (Miscellaneous / Apple “Other”) may refine from **generic title words** (`TitleCategoryHints`: wireless, installment, games) — not a per-user merchant list. `CreditAnalytics` matches checking ACH to the card Thank You by last-four or issuer (`CardIssuerCatalog`).

### Accounts

`CardsView` builds `AccountsBoard` (`AccountsBoard.swift`) from transactions, accounts, and payments. `CreditAnalytics.deduplicated` collapses checking ACH + card Thank You within 3 days when they share a card identity (mask / Apple / Amex); EPAY without a mask pairs with the unmatched card-side of the same amount. Detail: `CardDetailView.swift` (nickname via `CardLabelStore.swift`, debit/ACH rails via `PaymentRail.swift`, rewards via `CardBenefitsStore.swift` + `CardProductCatalog.swift`, payoff plans via `PayoffPlanEditorView.swift`). Purchase APR **0%** pre-fills a Promo APR plan. Logos: `Shared/UI/BankIcon.swift` + `InstitutionLogoCache`.
