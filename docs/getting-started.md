...
1. On a Mac, open **`FinanceWidget.xcodeproj`** (inside the git root).
2. Wait for indexing to finish.
3. Select the **FinanceWidget** scheme (not only the widget extension).
4. Pick a simulator or a physical device.

## 2. Signing

1. Select the **FinanceWidget** target → **Signing & Capabilities**.
2. Choose your **Team** (Personal Team works for development).
3. Repeat for **WidgetExtension** (must sign with the same team).
4. Bundle IDs default to something like:
   - App: `net.roberth.FinanceWidget`
   - Widget: `net.roberth.FinanceWidget.Widget`

If bundle IDs change, update App Group capability consistency (next section).
...
```text
group.net.roberth.FinanceWidget
```

Check:

- **FinanceWidget** target → Signing & Capabilities → **App Groups**
- **WidgetExtension** target → same group checked

Entitlements files:

- `FinanceWidget/FinanceWidget.entitlements`
...
1. **Build and run** the **FinanceWidget** app (⌘R).
