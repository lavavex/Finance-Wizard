...
enum DebugDataExporter {
    ...
    static func exportPackage(modelContext: ModelContext) throws -> URL {
        ...
        // try? swallows zip failures and falls back to returning the folder URL.
    ...
    /// App/version/schema metadata section of the snapshot.
    ...
    /// Row counts for a quick overview without scanning every array.
    ...
    private struct AccountDTO: Encodable {
        ...
        // Liabilities (credit-card billing fields from Plaid when available)
        var isOverdue: Bool?
    ...
    /// Export shape for an Income row.
    ...
    /// Export shape for a credit-card payment record.
    ...
    /// Linked Plaid Item metadata without the secret access token value.
    ...
    private static func buildSnapshot(modelContext: ModelContext) throws -> Snapshot {
        ...
        // Flag presence of secrets without exporting their values.
    ...
    /// enumerator walks the folder tree; relative paths become ZIP entry names.
    ...
    /// Local timestamp for folder/file names (e.g. 20240115-143022).
    private static func timestamp() -> String {
        let f = DateFormatter()
        // en_US_POSIX avoids locale surprises for fixed formats (recommended for machine strings).
    ...
    /// ISO-8601 string of “now” for meta.exportedAt.
...
/// “Store” means files are packed without compression — simpler and fine for JSON/text.
private enum MinimalZip {
    /// Builds ZIP binary from named file payloads (local headers + central directory + end record).
    static func pack(entries: [(name: String, data: Data)]) throws -> Data {
        var localFiles = Data()
        var centralDirectory = Data()
        // offset tracks byte position of each local file header in the output.
        var offset: UInt32 = 0
        ...
        for entry in entries {
            ...
            // Local file header — magic numbers are part of the ZIP file format spec.
            var local = Data()
            ...
            // Central directory header — index used by unzip tools to find each file.
            var central = Data()
        ...
        // End of central directory record — required trailer for a valid ZIP.
        var end = Data()
    ...
    /// Little-endian 16-bit integer as two bytes (ZIP format is little-endian).
    private static func u16(_ v: UInt16) -> [UInt8] {
        [UInt8(v & 0xff), UInt8((v >> 8) & 0xff)]
    }

    /// Little-endian 32-bit integer as four bytes.
    ...
    /// CRC-32 (ISO 3309 / ZIP) checksum used by ZIP to detect corruption.
